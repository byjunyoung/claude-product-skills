...
- **`/fig:lint` has a new `[mode]` check, graded blocking.** The login screens were cloned from a screen that looked light only because the page it sat on set the mode; on a page that set nothing they fell back to the collection default, which was dark. The check is per screen: where a screen's colours are bound to a collection with several modes and neither the screen, its section nor its page names one, it is reported. A version that compared each screen with its siblings was tried first and dropped: on the page where it happened, every screen had been cloned the same way, so there was no sibling to disagree with. It fires on screens that are correct today by relying on the default, too — deliberately, because the default is exactly what a move silently changes
- **`/fig:draw` now runs `/fig:tokens` on every drawn frame, not only on what was built from scratch.** The warning panels were clones, which the old rule exempted. Their source was a draft component built earlier in the same work, with its text colour typed in by hand for the dark mode. Cloning copied it faithfully
- **And it renders every drawn frame in one image before reporting.** A dark screen among light ones, an empty box, a hover-only control left on — each looks fine alone and wrong beside its siblings. Lint and tokens measure what they can name; the side-by-side render is where the rest turns up

## 3.20.0 — 2026-09-23

- **A complete spec entry still did not let anybody picture the screen.** On a layout change — a dialog becoming a side panel that pushes the page aside — the direction was settled in three broad strokes, the rest filled with the drafter's own recommended values, and a full spec draft put up for a go. It came back twice: first because the direction had never actually been agreed, then, once each point had been settled one by one, because a table of rules still did not show what the screen would do. What finally landed was a rough text wireframe of each moment the screen changes, and a tree of which state follows which. `/fig:draw` now does that before anything reaches the file whenever a screen is about to look or behave differently: the list of decisions first, one question at a time with a recommendation, a table of what was settled, a sketch per state, a transition tree, and what differs from today. Box labels stay ASCII with a legend outside, because a wide character takes two columns in a monospace terminal and breaks every right-hand border. It sits ahead of preview → go rather than replacing it, and a stub the canonical screen already settles is exempt
