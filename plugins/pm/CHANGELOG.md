...
- **What precedent turns up is folded into the recommendation, not delivered as a separate finding.** Its source is logged in the entry's References under a `Precedent —` line, apart from whatever backs a stated fact there — the two read differently, and blending them would make an inspiration look like evidence for something the entry claims. A policy entry does not get this check; it is usually a business or legal call, not a shape to compare against another product's
- **New config:** `prd.precedent.enabled` (default `true`) turns the whole check off where a team does not want it; `prd.precedent.archive_dir` names a local research archive to check first, and ships `null`
- `WebSearch` joins the skill's allowed tools

## 0.26.0 — 2026-09-21
- **The vocabulary check only ever read roles.** Item 0 of `/pm:prd`'s consistency pass collects the role and account words and looks each one up in the user-group table, which is why a document naming the same account three ways gets caught. A product's own states — what a machine, a screen or an order is in right now — had no table to be looked up in, and `structure.cases` is not one: those rows are the shapes any screen can be in, identical in every document. So three entries naming the same six states three different ways passed, and then came back under item 1 as contradictions that were never contradictions
- **Item 0 now covers both, and says where the state vocabulary lives.** One policy entry holds it, written in `structure.policy_sections` like any other, and the entries that use those names are read against it. Where the names have already drifted and no such entry exists, the skill proposes one rather than reconciling the entries against each other pair by pair — that work grows with how many entries there are and leaves nothing behind for the next one somebody writes
- **And a sixth check: an entry claiming to describe what already ships, against what actually ships.** The other five read the entries against each other; this one reads an entry against the thing it describes, and only those entries. One proposing something not built yet is supposed to differ from what ships, and a check that flags those is a check nobody reads twice
- **What it finds is reported, never reconciled.** A build drifts from an approved decision as easily as a document drifts from a build, and an entry quietly rewritten to match the code is how an approved decision disappears with nobody deciding to drop it. Both sides get quoted with the fact that would settle it — and where the material carried no code or build, the check says it did not run rather than leaving the entries looking confirmed
- **And the interview builds its question list from more than the blank fields.** A blank is a question the format already knew to ask; the ones that cost a second round are the ones an answer creates — adopt *it can be deleted* and what becomes of what was already there, and what somebody on another screen sees, are both decisions no field was holding. Each recommendation about to go up is now read for what adopting it would settle next, and that joins the same pass instead of waiting for the consistency check to find it after a draft exists
- No new config key. All of it reads `structure.policy_sections` and `structure.cases`, which are already there
