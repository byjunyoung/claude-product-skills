...
`task-publish`는 한 건을, `task-sync`는 목록을 맡습니다. 나눠 둔 이유는 실패하는 방식이 다르기 때문입니다. 한 건은 잘못 등록돼서 망가지고, 목록은 어긋나면서 망가집니다 — 미등록, 중복, 부모 오연결, 부활, 그리고 완료 조건이 다 체크됐는데 진행률은 아직 25%라고 적혀 있는 것. 어긋남을 진단하려면 양쪽을 먼저 읽어야 해서, `sync`는 쓰지 않고 진단을 먼저 보여준 뒤 멈춥니다.
