...
| `/fig:prep` `/fig:draw` `/fig:lint` `/fig:sync` `/fig:diff` `/fig:qa` | **Notion** — 설정이 Notion 페이지를 가리킬 때만 |
...
`fig`은 여럿이 함께 쓰는 Figma 파일을 위한 도구입니다. 리듬이 셋입니다. 기능 하나를 그려 넘기는 한 사이클, 릴리스가 나간 뒤 정본을 맞추는 주기 작업, 그리고 아무 때나 돌리는 건강검진.
...
/fig:draw     껍데기를 하나씩 채운다 — 파일이 쓰는 패턴을 따라, 정본 화면을 복제해서
...
`draw`가 그 껍데기를 채우는데, 혼자 그리지 않습니다 — 껍데기가 속한 화면을 통째로 복제해서, 결과물이 실제 최종 화면이 되게 합니다. 그 상태에 쓸 컴포넌트가 디자인시스템에 이미 있으면 그것부터 재사용하고, 아직 정해지지 않은 사실이 하나 있을 뿐이면 나머지는 실제로 그리고 그 사실 하나만 해당 노드에 주석으로 남깁니다 — 화면 전체를 빈 상자로 남겨두지 않습니다.

그리기 전에 건너뛰기 쉬운 질문을 먼저 던집니다. **이 파일은 이런 걸 이미 어떻게 짜고 있나?** 일관성이 깨지는 건 대개 부주의 때문이 아닙니다. 알림 행을 오늘 하나 그리고 보름 뒤에 또 하나 그리면 둘 다 그럴듯하지만 같지는 않습니다 — 그 행을 이 파일에서 어떻게 짜는지 말해주는 게 없었으니까요. 디자인시스템이 갖고 있는 건 부품이고, 부품의 배열은 갖고 있지 않습니다. 그래서 `draw`는 그 규칙을 별도 페이지에서 찾습니다 — 반복 상태를 모아둔 페이지 옆에 두는 **패턴 페이지**입니다. 규칙이 있으면 따르고, 두 화면이 이미 같은 방식이면 그 규칙을 적어두고, 아무것도 답하지 않으면 멈추고 정하게 합니다. 그 결정은 이 화면 하나가 아니라 이후의 모든 화면을 구속하기 때문입니다.
...
flowchart TD
    ...
    draw["fig:draw<br/>패턴 → 정본 복제 → 껍데기 채우기"]
...
| `/fig:draw` | 상태별 러프 텍스트 스케치로 방향을 먼저 합의한 뒤, 파일이 쓰는 패턴을 따라 가장 가까운 정본 화면을 복제해 화면을 그리거나 껍데기를 채운다 |
...
| 무엇을 | 어떤 문제를 잡나 |
|---|---|
| 구조 | 화면이 섹션 밖으로 빠짐 · 섹션 경계 이탈 · 화면끼리 겹침 · 이름 규칙 위반 · 섹션 순번과 배치 불일치 · 아무도 정하지 않은 라이트·다크 모드로 그려지는 화면 · 제 칸에 잘리거나 화면 밖으로 삐져나간 요소 |
...
모드 검사는 원래 있던 자리 덕분에 멀쩡해 보이던 복제 화면을 잡습니다. 여러 모드(라이트·다크 같은)를 가진 토큰에 묶인 색은 화면·섹션·페이지 중 가장 가까운 곳이 정한 모드로 그려지고, 아무 데도 안 정했으면 토큰의 기본 모드로 그려집니다. 라이트를 정해 둔 페이지에서 아무것도 안 정한 페이지로 화면을 옮기면 다크로 돌아오는데, 노드만 봐서는 이상한 데가 없습니다. 그래서 그런 토큰을 쓰는 화면은 기본값을 원하더라도 위쪽 어딘가에 모드를 적어 둬야 합니다.
...
**그리고 티켓은 그 일을 할 사람이 읽으라고 씁니다.** 티켓을 여는 사람 대부분은 그게 올라앉은 시스템을 읽지 않습니다 — 화면을 그리는 사람, 이번 버전에 무엇을 넣을지 정하는 사람, 나간 뒤에 책임지는 사람. 그래서 제목은 내부에서 쓰는 말 대신 무엇이 달라지는지를 적고, 조문 번호나 스펙 경로는 그걸 설명하는 조건 안이 아니라 비고 절에 둡니다. 레코드 제목이 그 내부 어휘로 적혀 있으면 미리보기가 그 옆에 쉬운 이름을 같이 내놓습니다. 조용히 바꾸지 않고요. 다만 쉽게 쓴다고 문장이 되면 안 됩니다 — 제목은 할 일을 가리키는 명사구이고, 그 목록의 다른 행들이 이미 쓰고 있는 말꼴을 따릅니다. 증상을 문장으로 적은 건 결함 제목이지 일감 제목이 아닙니다. `/pm:task-sync`는 목록 전체에서 같은 어긋남을 찾아 보고만 합니다 — 남이 쓴 문장을 승인 한 번으로 고쳐 쓰는 것, 그건 어느 쪽도 하지 않습니다.
...
**빈칸이 하나도 없는 문서도 틀릴 수 있습니다.** 그래서 다 쓴 항목들을 서로 대조하고, 그다음 항목이 설명하는 대상과 대조합니다 — 서로 어긋나는 두 항목, 갈래가 둘인데 결과가 하나인 조건, 한 칸에 두 경우가 들어 있는 케이스 행, 셀 수 없는 임계값, 켜는 쪽만 적히고 끄는 쪽이 빈 규칙. 역할과 그 제품 고유의 상태 이름을 먼저 봅니다. 어긋나 보이는 두 항목이 실은 같은 것을 두 이름으로 부른 경우가 그만큼 많고, 그건 어휘를 담은 항목이 가려 주지 추측으로는 안 갈리기 때문입니다. 마지막으로 「지금은 이렇게 동작한다」고 적힌 항목을 실제로 나가 있는 것과 대조합니다. 나온 것은 양쪽을 인용하고 무엇이 판정할지를 적어 보고하지, 코드 쪽으로 맞추지 않습니다 — 문서가 빌드에서 밀려나는 만큼 빌드도 승인된 결정에서 밀려납니다.
...
설정으로 맞출 건 이 둘뿐이고, 각각 이유가 있습니다. 기획서는 같은 기능의 양쪽 끝에서 두 번 읽힙니다. `/fig:draw`는 화면을 그리는 동안 읽습니다 — 파일에 들어가는 문구가 라벨을 채우려고 지은 그럴듯한 말이 아니라 문서가 정한 말이 되도록. `/fig:qa`는 그 뒤에 읽습니다 — 결함 보고가 「Y 문서의 X 규칙을 어겼다」로 읽힐 때만 넘길 값이 있기 때문입니다. 그 Y 문서를 쓴 게 `/pm:prd`입니다. `/fig:diff`에 일감이 필요한 건 AS-IS/TO-BE 표가 그걸 부른 요청 옆에 있어야 하기 때문입니다 — 그 레코드를 연 게 `/pm:task-draft`입니다.

앞쪽 한 번이 생각보다 중요합니다. 티켓의 완료 조건은 시안의 구조와 문구에서 나옵니다. 그리다가 지어낸 라벨 하나가 개발이 그걸 보고 만들고 체크하는 한 줄이 된다는 뜻입니다. 그리는 동안 기획서를 읽는 게 추측이 요구사항으로 굳는 걸 막는 장치입니다.

둘 다 필수는 아닙니다. `qa.baseline.prd`를 **비워 두면**(`null` — 이 설정에서 「안 정함」을 뜻합니다. 비운 항목은 검사를 실패시키지 않고 건너뜁니다) `/fig:qa`는 전부 결함이 아니라 확인 필요로 올리고, `/fig:draw`는 원천을 가정하지 않고 어디서 가져올지 묻습니다. `task_tracker.type`을 `none`으로 두면 `/fig:diff`는 표를 마크다운으로 뱉습니다. 잃는 건 연결이지 기능이 아닙니다.
