...
**A document with no blank left in it can still be wrong.** So the finished entries are read against each other, and then against the thing they describe: two that contradict, a condition with two branches and one outcome, a case row holding two conditions in one cell, a threshold nobody can count, a rule written only in the direction where something is switched on. Roles and the product's own state names come first, because two entries that look like they disagree are as often one thing under two names — which the entry holding that vocabulary settles and guessing does not. Last, an entry saying *this is how it works today* is read against what actually ships. Whatever turns up is quoted from both sides with the fact that would settle it, and nothing is reconciled toward the code: a build drifts from an approved decision as easily as a document drifts from a build.
